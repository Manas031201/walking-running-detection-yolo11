# Human Walking & Running Detection using YOLO11-Pose 🏃‍♂️🚶‍♂️

An automated computer vision system built on Google Colab that detects a human subject, tracks their movement using bounding boxes, and dynamically classifies whether they are **Standing**, **Walking**, or **Running** using deep-learning pose estimation.

---

## 🚀 Key Features
* **YOLO11 Pose Core:** Utilizes the state-of-the-art `yolo11n-pose.pt` model for incredibly fast, light-weight inference.
* **Biometric Joint Tracking:** Specifically monitors **Ankles (COCO Keypoints 15 & 16)** because they demonstrate the most distinct positional displacement during human locomotion.
* **Mathematical Vector Classification:** Employs Euclidean distance calculations ($||A_{current} - A_{previous}||$) to instantly calculate structural movement velocities between consecutive frames.
* **Adaptive Aspect-Ratio Resizing:** Includes automated frame-downscaling logic to perfectly fit within the Google Colab display pane without cropping out elements.

---

## 🛠️ How It Works (The Logic)
The framework monitors lower-body coordinates over successive frames. Pixel distance thresholds dictate the movement categories:
1. **Displacement < 2.5 pixels:** Subject is stationary or swaying mildly $\rightarrow$ **Standing**
2. **Displacement between 2.5 and 20 pixels:** Rhythmic, continuous pacing $\rightarrow$ **Walking**
3. **Displacement > 20 pixels:** Rapid, high-velocity lower body acceleration $\rightarrow$ **Running**

---

## 💻 Step-by-Step Guide to Run on Google Colab

### 1. Environment Setup
Open a clean notebook in [Google Colab](https://colab.research.google.com/) and run the following command in your first cell to install dependencies:
```bash
